...
function matchStops(sched, name) {
  ...
  for (const s of sched.stops) {
    const sn = (s.stop_name || "").toLowerCase().replace(/[^a-z]/g, "");
    if (sn.includes(n) || n.includes(sn)) ids.add(s.stop_id);
  }
  return ids;
}
 ...
 */
export async function vreDepartures({ from, to, limit = 4, now = Date.now() }) {
  ...
  return { from, to, departures: out.slice(0, limit), feedTrips: updates.length };
}
